 ...
 */
	...
	$.jstree.core.prototype = {
		...
		_open_to : function (obj) {
			...
			return $('#' + obj.id.replace($.jstree.idregex,'\\$&'), this.element);//跳回2926
		},
		/**
		...
		select_node : function (obj, supress_event, prevent_open, e) {
			...
			if(!obj.state.selected) {//obj就是父亲节点即PDU PDU.state.selected=false
				obj.state.selected = true;
				this._data.core.selected.push(obj.id);
				if(!prevent_open) {//prevent_open=false
					dom = this._open_to(obj);//跳到2554
				}
				if(dom && dom.length) {
					dom.attr('aria-selected', true).children('.jstree-anchor').addClass('jstree-clicked');//选中了PDU，转2939
				}
				/**
				 ...
				 */
				  //这句话把所有的子节点都选中了
				// var childrenId=this._data.core.selected;
		...
		create_node : function (par, node, pos, callback, is_loaded) {
            ...
            console.log(par);
            console.log(pos);
            console.log(this.check("create_node", node, par, pos));
			...
			if(this.settings.checkbox.cascade.indexOf('up') !== -1 || this.settings.checkbox.cascade.indexOf('down') !== -1) {
				this.element
					...
					.on(this.settings.checkbox.tie_selection ? 'select_node.jstree' : 'check_node.jstree', $.proxy(function (e, data) {
							...
							// apply down
							if(s.indexOf('down') !== -1) {
								// console.dir(this._data.core.selected);
							...
							if(s.indexOf('down') !== -1 && dom.length) {
								...
								var selectedChildrenId = [];
								selectedChildrenId = this._data.core.selected;
								for(var i=0;i < selectedChildrenId.length;i++){
									var id = selectedChildrenId[i];
									var flag = $("#"+id).attr('aria-disabled');
									if(flag == "true"){
										var idChild = id + "_anchor";
										$("#"+idChild).removeClass('jstree-clicked');
									}
								}
								selectedChildrenId = [];
							}
